...
def main():
    ...
    noise = []
    for i in range(20):
        xa = np.random.uniform(0, 0.2, (1000, 1))
        xb = np.random.uniform(0.2, 0.8, (1000, 1))
        c = np.random.choice(2, (1000,1), p = [0.2, 0.8])
        x1 = xa*c + xb * (1-c)
        x2 = np.random.uniform(0, 1, (1000, 5))
    ...
    for B in B_list:
        ...
        for C in C_list:
            ...
            np.save("./hartmann_imbalanced_loss_B_%d_T_%d_C_%d_discrete"%(B, T, C*100), L_trials)
            np.save("./hartmann_imbalanced_error_B_%d_T_%d_C_%d_discrete"%(B, T, C*100), error_list)
