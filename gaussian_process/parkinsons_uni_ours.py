...
def main():
    data = np.load("./parkinsons_uniform_data.npy")
    label = np.load("./parkinsons_uniform_label.npy")
    ...
    for B in B_list:
        for C in C_list:
            ...
            np.save("./parkinsons_uniform_loss_B_%d_T_%d_C_%d_ours"%(B, T, C*100), L_trials)
            np.save("./parkinsons_uniform_error_B_%d_T_%d_C_%d_ours"%(B, T, C*100), error_list)
